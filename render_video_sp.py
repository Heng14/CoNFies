...
def main(argv):
    ...
    aus_str = '_'.join([str(i) for i in aus])
    ...
    while True:
        ...
        for beta in betas:
            if model.use_attribute_conditioning or FLAGS.use_lr:
                ...
                attribute_set = jnp.zeros((num_attributes), dtype=jnp.float32) - 1.0
                for auid, attribute_index in enumerate(aus):
                    attribute_set = attribute_set.at[attribute_index].set(1.0)
