...
def main(argv):
    ...
    attribute_loss_weight_sched = schedules.from_config(   # by heng attr loss sched 
        train_config.attribute_loss_weight_schedule
    ) 
    ...
    scalar_params = training.ScalarParams(
        ...
        attribute_loss_weight=attribute_loss_weight_sched(0), #train_config.attribute_loss_weight, # by heng attr loss sched 
    ...
    for step, batch in zip(
        ...
        scalar_params = scalar_params.replace(
            ...
            attribute_loss_weight=attribute_loss_weight_sched(step),
        )
        ...
        if step % train_config.print_every == 0 and jax.process_index() == 0:
            logging.info(
                "step=%d, nerf_alpha=%.04f, warp_alpha=%.04f,  lr=%.06f, %s",
                step,
                nerf_alpha_sched(step),
                warp_alpha_sched(step),
                learning_rate_sched(step),
