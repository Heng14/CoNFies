...
def generate_points_bk(num_attributes: int):
...
def generate_points_bk1(num_attributes: int):
    aus = [1, 9]
    points = [[MIN_VALUE] * num_attributes, [MAX_VALUE] * num_attributes]
    for i in range(num_attributes - 1, -1, -1):
        new_point = points[-1][:]
    ...
    mask = np.zeros_like(points_array)
    for au in aus:
        mask[:, au] = 1
...
def generate_points(num_attributes: int):
    aus = [2, 15]
    points = [[MIN_VALUE] * num_attributes]
    for au in aus:
        new_point = [MIN_VALUE] * num_attributes
        new_point[au] = MAX_VALUE
        points.append(new_point)
        new_point = [MIN_VALUE] * num_attributes
        points.append(new_point)
