...
class InterpDataSource(core.DataSource):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        annotations = {}
        annotations_given = {}
        self.value_len = 1 #2 #3 # hard code by heng, need change between 1or2 mask
                ...
                for entry in annotations_in_file:
                    ...
                    if entry["frame"] not in annotations:
                        annotations[entry["frame"]] = np.zeros(
                            (self.num_attributes * self.value_len, ), dtype=np.float32
                        )
                        annotations_given[entry["frame"]] = np.zeros(
                            (self.num_attributes * self.value_len,), dtype=np.float32
                        )
                    annotations[entry["frame"]][entry["class"] * self.value_len:(entry["class"]+1) * self.value_len] = np.array(entry['value'])
                    # annotations_given[entry["frame"]][entry["class"]] = 1
                    annotations_given[entry["frame"]][entry["class"] * self.value_len:(entry["class"]+1) * self.value_len] = 1
    ...
    def load_attribute_is_given(self, item_id: str) -> np.ndarray:
        return self.annotations_given.get(
            int(item_id),
            np.zeros((self.num_attributes * self.value_len,), dtype=np.float32),
        )

    def load_attribute_values(self, item_id: str) -> np.ndarray:
        return self.annotations.get(
            int(item_id), np.zeros((self.num_attributes * self.value_len, ), dtype=np.float32)
        )
