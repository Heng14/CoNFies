...
def train_step(
    ...
    def _compute_loss_and_stats(
        ...
        if use_attribute_loss:
            ...
            attributes_uncert = model_out['uncertainty_rgb']            

            attribute_loss = (
                (
                    ( (model_out["hyper_projected_embed"] - attributes_gt_values)   
                    ** 2 ) / (2 * attributes_uncert ** 2) + (jnp.log(attributes_uncert) ** 2) / 2       # uncertainty
                )
            ...
            stats["loss/attribute_loss_weight"] = (  # by heng attr loss sched 
                scalar_params.attribute_loss_weight
            )

            stats["loss/attribute_uncertainty"] = (  # by heng attr loss sched 
                ( (attributes_uncert ** 2) * batch["metadata"]["attribute_is_given"]).sum() / (batch["metadata"]["attribute_is_given"].sum())
            )
